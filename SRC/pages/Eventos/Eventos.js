...
const addFavorito = async (evento) => {
  ...
  const res = await fetch(`http://localhost:3000/api/v1/${user._id}`, opciones);
...
const addFavorito = async (evento) => {
  ...
  if (res.ok) {
    ...
    mostrarAlerta("Evento añadido a favoritos");

    actualizarMisEventos(evento);
  } else {
    console.log("Error al añadir a favoritos");
    mostrarAlerta("Error al añadir el evento a favoritos");
  }
};
